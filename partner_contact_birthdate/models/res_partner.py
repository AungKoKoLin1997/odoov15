...
class ResPartner(models.Model):
    ...
    def birthday_mail_send(self):
        partners = self.env['res.partner'].search([('birthdate_date','!=',False),('email','!=',False)])
        ...
        for partner in partners:
            ...
            if months == 0 and days ==0:
                ...
                template_id = self.env.ref('partner_contact_birthdate.mail_template_birthday_wish').id  #Get mail template
                template = self.env['mail.template'].browse(template_id)
                template.write({'email_to':partner.email}) #Write the email address of partner
